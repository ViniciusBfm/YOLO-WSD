from flask import Flask, render_template, jsonify, request, redirect, url_for, session, flash, after_this_request, send_from_directory, make_response, Response
import sqlite3
...
@app.route('/relatorio_notificacoes')
@login_required
def gerar_relatorio_notificacoes():
    # Recuperar notificações do banco de dados
    conn = sqlite3.connect('controle.db')
    cursor = conn.cursor()
    cursor.execute("SELECT timestamp FROM capturas_cam1")
    notificacoes = cursor.fetchall()
    conn.close()

    # Criar um DataFrame do Pandas com os dados
    df = pd.DataFrame(notificacoes, columns=['timestamp'])
    df['timestamp'] = pd.to_datetime(df['timestamp'], format='%d/%m/%Y %H:%M:%S')
    df['mes_ano'] = df['timestamp'].dt.to_period('M')
    print(df['timestamp'])

    # Contar a frequência de cada mês_ano
    contagem_mes = df['mes_ano'].value_counts().sort_index()

    # Gerar o gráfico de barras com Matplotlib para notificações por mês
    plt.figure(figsize=(10, 6))
    plt.bar(contagem_mes.index.astype(str), contagem_mes.values, color='skyblue')
    plt.xlabel('Mês e Ano')
    plt.ylabel('Número de Notificações')
    plt.title('Número de Notificações por Mês em 2024')
    plt.xticks(rotation=45)
    plt.tight_layout()

    # Salvar o gráfico em um buffer de memória
    img_buffer_mes = io.BytesIO()
    plt.savefig(img_buffer_mes, format='png')
    plt.close()
    ...
    # Adicionar o título ao PDF
    title = Paragraph('Relatório de Notificações', title_style)

    # Adicionar os gráficos ao PDF
    img_mes = Image(img_buffer_mes, 6*inch, 4*inch)  # Ajuste o tamanho conforme necessário

    # Organizar os elementos do PDF
    elementos = [title, img_mes]
    ...
    # Criar uma resposta Flask com o PDF
    response = make_response(buffer.read())
    response.mimetype = 'application/pdf'
    response.headers['Content-Disposition'] = 'inline; filename=relatorio_notificacoes.pdf'
...
def notificacoes():
    ...
    # Consulta SQL para obter os 12 dados mais recentes da tabela capturas_cam1 ordenados por timestamp em ordem decrescente
    cursor.execute("SELECT * FROM capturas_cam1 ORDER BY id DESC LIMIT 12")
    notificacao = cursor.fetchall()
    conn.close()

    return render_template('notificacao.html', notificacao=notificacao, nome_usuario=nome_usuario, opcoes_funcao=opcoes_funcao, funcao_do_usuario=funcao_do_usuario)
...
@app.route('/galeriadeimagens')
@app.route('/galeriadeimagens/<int:pagina>')
def galeriadeimagens(pagina=1):
    if not current_user.is_authenticated:
        return redirect(url_for('login'))
    nome_usuario = current_user.nome
    funcao_do_usuario_simples = current_user.funcao
    ...
    offset = (pagina - 1) * imagens_por_pagina

    # Consulta SQL para obter os dados da tabela capturas_cam1 com limite, offset e ordenação
    conn = sqlite3.connect('controle.db')
    cursor = conn.cursor()
    cursor.execute("SELECT COUNT(*) FROM capturas_cam1")
    total_imagens = cursor.fetchone()[0]

    cursor.execute("SELECT * FROM capturas_cam1 ORDER BY id DESC LIMIT ? OFFSET ?", (imagens_por_pagina, offset))
    registros = cursor.fetchall()
    conn.close()

    total_paginas = (total_imagens + imagens_por_pagina - 1) // imagens_por_pagina

    return render_template('galeriadeimagens.html', registros=registros, pagina=pagina, total_paginas=total_paginas,
                           nome_usuario=nome_usuario, opcoes_funcao=opcoes_funcao, funcao_do_usuario=funcao_do_usuario)

@app.route('/remover_imagem/<int:imagem_id>', methods=['POST'])
@login_required
def remover_imagem(imagem_id):
    ...
    cursor.execute("DELETE FROM capturas_cam1 WHERE id = ?", (imagem_id,))
    conn.commit()
    conn.close()

    return redirect(url_for('galeriadeimagens'))
